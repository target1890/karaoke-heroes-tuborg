...
const StaffettaMode = ({
  ...
  return <div className="min-h-screen bg-cover bg-center relative p-8" style={{
    backgroundImage: `url(${sfondo})`
      ...
      <Button onClick={onAddPoints} className="absolute top-24 right-8 z-20 text-primary-foreground px-6 py-3 rounded-xl shadow-lg border-2 border-white text-base font-bold bg-[#440c7f]">
        ➕ Aggiungi Punti
      </Button>
      ...
      <div className="relative z-10 pt-28 pb-12 max-w-7xl mx-auto">
        ...
        textShadow: category === "classic" ? '0 0 20px #ffff00, 0 0 40px #ffff00, 0 0 60px #ffff00' : '0 0 20px #00ff00, 0 0 40px #00ff00, 0 0 60px #00ff00'
      }}>
          {category === "classic" ? "🟡 Classic" : "🟢 Fresh"} - Staffetta
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 px-4 py-8">
          {songs.map((song, index) => {
            ...
            return (
              ...
              >
                <span className={`font-black text-xl ${isPlayed ? "text-red-600" : category === "classic" ? "text-yellow-600" : "text-primary"}`}>
                  {song.title}
                </span>
                <span className={`text-sm font-semibold ${isPlayed ? "text-red-700" : category === "classic" ? "text-yellow-700" : "text-secondary"}`}>
                  {song.artist}
                </span>
              </Button>
            );
